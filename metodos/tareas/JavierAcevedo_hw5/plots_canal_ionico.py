...
#Primera figura.
plt.scatter([px0], [py0], color = 'r')
c0 = plt.Circle((px0,py0), R0, fill = False)
ax.add_artist(c0)
plt.scatter(x1,y1)
plt.title(('Parametros: x = %.3f, y = %.3f, R = %.3f' %(px0, py0, R0)))
plt.savefig("g1.png")
...
ax.add_artist(c1)
plt.scatter(x2,y2)
plt.title(('Parametros: x = %.3f, y = %.3f, R = %.3f' %(px1, py1, R1)))
plt.savefig("g2.png")


#Histogramas
noH = plt.figure()
plt.hist2d(res0[:,0], res0[:,1], bins = 100,)
#plt.hist2d(res0[:,0], res0[:,1], bins = 100, range = [[-5,16], [-5,20]])
plt.title('Histograma Primer Archivo')
#plt.hist(res0[:,0], bins = 100)
plt.savefig("his1.png")

noHindeed = plt.figure()
plt.hist2d(res1[:,0], res1[:,1], bins = 200)
plt.title('Histograma Segundo Archivo')
